...
Simplifies governance and management of hybrid and multi-cloud environments. Extends Azure Compliance and Monitoring to non-azure resources.
Manage onprem and multi-cloud resources (VMs, Kubernetes Clusters, databases) as though they were in Azure.
Enables use of Azure services and management capabilities across different environments.
Supports managing wide range of resources (servers, kubernetes, Azure data services, SQL server, VM's...)
...
Azure Portal: web based, unified console. Manage Azure subscriptions with a [[id:de12ea67-0c1c-4772-aff3-05bab32ba28d][GUI]]. Offers custom dashboard, continuous availability, etc.
Azure cloud shell: browser based shell, supports Azure Powershell and Azure CLI. Manage Azure resources remotely.
Azure Powershell: shell for running commands (cmdlets) to manage Azure resource, available via Cloud Shell. Multi-platform.
Azure CLI: similar to Azure Powershell, uses bash commands (same functionality)
** Functionality and Usage of Azure Advisor - [[https://youtu.be/nqH4NboyEl0&t=3m22s][video]]
Evaluates your Azure resources, and provides recommendations to improve reliability, security, performance, and cost.
Recommendations are available via the Azure portal, and can be accessed via the Azure Advisor API.
Dashboard displays personalized recommendations for all subscriptions and resource groups. Can filter for specific resources, and can dismiss recommendations that are not relevant.
Recommendations are divided into 'reliability', 'security', 'performance', and 'cost' categories.
** Functionality and Usage of ARM Templates - [[https://youtu.be/loxcA5MUf-I&t=6m41s][video]]
Azure Resource Manager (ARM): deployment and management service for Azure. Handles requests from Azure tools, APIs, SDK's, and ensures consistent results.

Benefits: allows managing infrastucture through declarative templates, deploying resources as a group, and applying access control and tags for better organization and billing.
Templates: JSON files describe resources in a declarative format. Ensures repeatability deployments and modularity.
Bicep: simpler, more concise language than JSON for deployiung Azure resources. Supports all resource types and API versions. 
** Functionality and Usage of Azure Monitor - [[https://youtu.be/v68jL-l9Fww&t=10m20s][video]]
Azure Monitor: collects, analyzes, and acts on telemetry from your cloud and on-premises environments. Provides a comprehensive solution for collecting, analyzing, and acting on telemetry from your cloud and on-premises environments.
Azure Log analytics: collects and analyzes data from multiple sources. Can be used to monitor and manage the performance of your applications and infrastructure.
Tool for writing and running queries against collected data. Can be used to create visualizations and alerts.
Azure Monitor Alerts: Automated notifications that are sent when a particular condition is met. Can be used to monitor the performance of your applications and infrastructure.
Application insights: Monitors web applications and services. Provides real-time insights into application performance and user behavior. Monitors across multiple platforms and languages.
...
Service health offers a more focused view of the health of the services in the regions that you use.
Resource health gives detailed information about the health of an individual resource, such as VM's and allows for alert configuration.
Historical Alerts stores alerts for 90 days, and allows you to view and manage alerts.
...
#  LocalWords:  multicloud NDA cmdlets SDK's VM's
